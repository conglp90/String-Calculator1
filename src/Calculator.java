...
public class Calculator {
	...
	int add(String input){
		...
		process_negatives(sohang_list);
		
		for(int sohang: sohang_list){
			kq+=sohang%1000; // 1003 <-> 3
			System.out.println("abc: "+sohang);
		}
		return kq;
	}

	private void process_negatives(ArrayList<Integer> sohang_list2) {
		String messendger="negatives not allowed: ";
		boolean isHasNegatives=false;
		for(int sohang: sohang_list){
			if (sohang<0) {
				if(isHasNegatives)
					messendger+=", "; 
		...
		if(isHasNegatives)
			throw new RuntimeException(messendger);
